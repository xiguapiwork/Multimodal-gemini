...
export async function fetchAndUploadFile(
  ...
  try {
    ...
    // upload() 返回的 file 对象包含 name, uri, state 等信息
    const initialUploadResponse = await ai.files.upload({
      file: blob,
      config: { mimeType },
    });
    const uploadedFile = initialUploadResponse.file;
    console.log(
      `File uploaded to Gemini. Name: ${uploadedFile.name}, URI: ${uploadedFile.uri}. Now waiting for it to become ACTIVE...`
    );

    // --- 新增：轮询文件状态，直到 ACTIVE 或 FAILED ---
    const MAX_RETRIES = 20; // 最多轮询 20 次 (约40秒)
    const POLLING_INTERVAL_MS = 2000; // 每 2 秒检查一次

    for (let i = 0; i < MAX_RETRIES; i++) {
      // 使用文件 'name' 而不是 'uri' 来获取文件状态
      const fetchedFileResponse = await ai.files.get({ name: uploadedFile.name });
      const currentFile = fetchedFileResponse.file;
      ...
      if (currentFile.state === "ACTIVE") {
        console.log(`File '${currentFile.name}' is now ACTIVE. Proceeding.`);
        // 文件已激活，返回这个最新的文件对象和它的mimeType
    ...
    throw new Error(
      `File '${uploadedFile.name}' did not become ACTIVE within the timeout period.`
    );
    // ---------------------------------------------------
